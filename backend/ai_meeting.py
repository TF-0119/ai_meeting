...
class Meeting:
    def __init__(self, cfg: MeetingConfig):
        ...
        self.equilibrium_enabled = self.cfg.equilibrium # type: ignore
    ...
    def _think(self, agent: "AgentConfig", last_summary: str) -> str:
        ...
        req = LLMRequest(system=sys, messages=[{"role":"user","content":user}], temperature=min(0.9, self.temperature+0.1), max_tokens=120)
        return self._enforce_chat_constraints(self.backend.generate(req)).strip()

    # --- 均衡AI（T1）: 思考を審査して勝者を選出（JSONで） ---
    def _judge_thoughts(self, bundle: Dict[str,str]) -> Dict:
        sys = "あなたは中立の審査員です。流れ適合・目的適合・質・新規性・実行性を0〜1で採点し、総合scoreを算出して勝者を1名だけ選びます。出力はJSONのみ。"
        # 思考一覧を安全に列挙（本文に混入させない）
        ...
        user = (
            ...
            "\n\nJSON形式で出力:\n{\n"
            '  "scores": {\n'
            '    "A": {"flow":0-1,"goal":0-1,"quality":0-1,"novelty":0-1,"action":0-1,"score":0-1,"rationale":"短文"},\n'
            '    "...": {...}\n'
            '  },\n'
            '  "winner": "A"\n'
            "}\n"
        ...
        for r in range(1, self.cfg.rounds+1):
            ...
            if self.cfg.think_mode:
                ...
                for ag in self.cfg.agents:
                    thoughts[ag.name] = self._think(ag, last_summary)
                ...
                self._last_spoke[winner.name] = global_turn
                ...
                self._last_spoke[speaker.name] = global_turn
            ...
            if self._monitor:
                ...
                if ev:  # フェーズ確定
                    ...
                    if self._shock_engine:
                        ctx = {"reason": ev.get("reason"), "summary": ev.get("summary",""), "topic": self.cfg.topic}
                        self._shock_hint, shock_note = self._shock_engine.generate(ctx)
                        self._shock_ttl = self.cfg.shock_ttl
                        ev["shock_used"] = shock_note or "(generated)"

                    self.logger.append_phase(ev) # type: ignore
                    # フェーズが変わっても “会議本文には何も挿入しない”（参加AIは気づかない）

            # ショックの寿命を進める（ターンの最後にデクリメント）
...
class KPIFeedback:
    ...
    def assess(self, turns: List[Turn], unresolved_hist: List[int]) -> Dict:
        ...
        if len(unresolved_hist) >= min(4, W):
            ...
            stall = all(recent[i] <= recent[i-1] for i in range(1,len(recent))) is False and \
                    (len(set(recent)) == 1)  # 全く減らない
